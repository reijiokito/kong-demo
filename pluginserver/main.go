...
/* flags */
var (
	prefix         = flag.String("prefix", "/home/cong/Documents/bnm/kong-demo/pluginserver", "")
...
func init() {
	flag.Parse()
	socket = *prefix + "/" + "go_pluginserver.sock"
...
func runServer(listener net.Listener) {
	var handle codec.MsgpackHandle
	handle.ReaderBufferSize = 10 * 1024 * 1024
	handle.WriterBufferSize = 10 * 1024 * 1024
	handle.RawToString = true
	handle.MapType = reflect.TypeOf(map[string]interface{}(nil))
...
func startServer() {
	...
	if err != nil && !os.IsNotExist(err) {
		log.Printf(`removing "%s": %s`, prefix, err)
		return
	}
...
func main() {
	...
	if socket != "" {
		go func() {
			for {
				if !isParentAlive() {
					log.Printf("Server exited; shutting down...")
					os.Exit(0)
				}
